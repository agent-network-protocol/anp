...
- **`group_e2ee/`**: ANP-P6 group E2EE SDK surface. It owns P6 wire models, recovery/update KeyPackage lifecycle shapes, group send/update AAD/JCS helpers, hidden leave-request/process control-plane structs, and shared contract fixtures. Real legacy OpenMLS execution is exposed through the typed Rust API in `rust/src/group_e2ee/operations/typed.rs` with local state behind `rust/src/group_e2ee/storage.rs`; the historical `anp-mls` binary/JSON command surface has been removed. The side-by-side P6 v2 wire/binding surface does not silently upgrade that legacy runtime. `rust/tests/group_e2ee_v2_multi_device_mls.rs` is the real-OpenMLS development gate for same-DID independent device leaves, exact-device removal, history/future exclusion, binding substitution rejection, and KeyPackage/private-state non-reuse while the draft codepoint remains release-blocked.
...
- Group E2EE callers use the typed functions in `anp::group_e2ee::operations`, not an `anp-mls` subprocess. `CompatDataDirStore` preserves the legacy `state.db` layout and `ImCoreSqliteGroupMlsStore` scopes state under an im-core root. MLS private state, KeyPackage private material, group bindings, idempotency records and pending commits stay local; decrypted plaintext is returned only from the active typed call. Real product flows must feed add/update/recover only message-service-verified and leased KeyPackages. Pending membership/update/remove commits finalize only after service acceptance or abort after deterministic rejection; OpenMLS 0.8 local leave remains terminal until an authorized active member advances the epoch. The existing typed runtime is legacy P6 v1 and must not be advertised as v2 merely because the separate v2 wire/binding helpers and integration gate exist.
