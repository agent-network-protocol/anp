...
- **`authentication/`**: DID WBA (Web-based Decentralized Identifiers) authentication — DID document creation (`create_did_wba_document`), auth header generation (`DIDWbaAuthHeader`), RS256 JWT signature verification (`DidWbaVerifier`), and the optional ANP vNext P2 `deviceManifest` typed parser/validator/device lookup surface. Existing v1 builders remain unchanged; durable Manifest SDK documentation lives in `docs/did/device-manifest-vnext-sdk.md`.
- **`direct_e2ee/`**: ANP-P5 private-chat E2EE SDK surface. Existing unprefixed Go/Rust/Python session APIs remain v1. Explicit side-by-side P5 v2 Rust/Python/Go/Dart wire models own device-qualified Bundle/RPC/AAD/plaintext validation and the `4000-4012` error table without reusing v1 Ratchet state. Rust additionally owns the v2 exact-device X3DH/Double-Ratchet runtime, distinct persisted state formats, exact-binding default selection, local peer-device disable primitive, stable runtime-to-P5 error mapping, and a deterministic per-session oldest-first `MAX_SKIP` bound across DH chains; product integrations still own current Manifest/Bundle preverification and atomic replay/session/OPK persistence. Other language v2 surfaces, including Dart, remain wire-only unless documented otherwise.
...
- **`group_e2ee/`**: ANP-P6 group E2EE SDK surface. It owns P6 wire models, recovery/update KeyPackage lifecycle shapes, group send/update AAD/JCS helpers, hidden leave-request/process control-plane structs, and shared contract fixtures. Real legacy OpenMLS execution is exposed through `rust/src/group_e2ee/operations/typed.rs`; the separate persistent P6 v2 path is `rust/src/group_e2ee/operations/v2.rs`. Both use local state behind `rust/src/group_e2ee/storage.rs`, but v1 state is never reinterpreted as v2. The v2 facade also exposes local-only, secret-free readiness and current DID/device endpoint inspection for product repair; neither is ANP wire or current authorization. P6 v2 Remove authenticates the exact target endpoint from the accepted OpenMLS tree so a device that has already lost current Manifest eligibility can still be removed; current P4 trigger and owner authorization stay outside the SDK. The historical `anp-mls` binary/JSON command surface has been removed. `rust/tests/group_e2ee_v2_multi_device_mls.rs` gates raw real-OpenMLS semantics and `rust/tests/group_e2ee_v2_operations.rs` gates the persistent facade, same-DID independent Leaves, exact-device removal, history/future exclusion, binding substitution rejection, KeyPackage/private-state non-reuse, and one MLS-encrypted attachment Manifest while the draft codepoint remains release-blocked.
- Durable Group E2EE SDK/OpenMLS documentation lives in `docs/e2e/group-e2ee-p6-anp-mls.md`; update it when P6 wire models, typed operations, local state, v2 cryptographic gates, or discovery posture changes.
...
- The independent vNext fixture `testdata/direct_e2ee/p5_v2_wire_vectors.json` is consumed by Rust, Python, Go, and Dart. Keep `anp_version`/`created_at` outside P5 AAD, `logical_message_id` inside ApplicationPlaintext only, explicit null out of optional wire members, and all AWiki-internal/root-control fields outside this public wire surface. Dart Bundle proofs must use Appendix-B base58-btc multibase and its canonical JSON must preserve RFC 8785 numeric bytes.
...
- Shared vNext Device Manifest fixtures live under `testdata/device_manifest/`; Rust, Python, Go, and Dart authentication tests must accept and reject the same closed Manifest schema, key relationship, Profile dependency, and current-device/key uniqueness cases without changing unknown DID top-level extensions.
...
- Group E2EE callers use typed functions in `anp::group_e2ee::operations`, not an `anp-mls` subprocess. `operations::typed` remains legacy P6 v1; `operations::v2` is the separate persistent P6 v2 facade and requires newly created v2 state. `CompatDataDirStore` preserves the legacy `state.db` layout and `ImCoreSqliteGroupMlsStore` scopes state under an im-core root. MLS private state, KeyPackage private material, group bindings and pending commits stay local; decrypted plaintext is returned only from the active typed call. V2 consumers should pass standard `V2GroupNoticeMetadata + V2E2eeNotice` to `process_notice_v2`; Commit sender/operation/subject bindings come from verified MLS AAD and current DID documents, not a caller-synthesized control DTO. Real product flows must use message-service-verified and leased KeyPackages and enforce P4 membership/owner policy and service CAS outside the MLS facade. Prepare/finalize uses a recoverable `preparing -> prepared -> accepted -> finalized/aborted` write-ahead journal because OpenMLS and metadata use separate SQLite connections; callers must run `reconcile_pending_v2` after restart. The journal never exports MLS private state. The v2 draft codepoint remains release-blocked even though local runtime development is supported.
