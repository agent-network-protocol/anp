...
- **`direct_e2ee/`**: ANP-P5 private-chat E2EE SDK surface. Existing unprefixed Go/Rust/Python session APIs remain v1. Explicit side-by-side P5 v2 Rust/Python/Go/Dart wire models own device-qualified Bundle/RPC/AAD/plaintext validation and the `4000-4012` error table without reusing v1 Ratchet state. Rust additionally owns the v2 exact-device X3DH/Double-Ratchet runtime, distinct persisted state formats, exact-binding default selection, and local peer-device disable primitive; other language v2 surfaces, including Dart, remain wire-only unless documented otherwise.
