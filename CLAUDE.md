...
- **`group_e2ee/`**: ANP-P6 group E2EE SDK surface. It owns P6 wire models, group send AAD/JCS helpers, and shared contract fixtures; real OpenMLS one-shot operations live in `rust/src/bin/anp-mls.rs`, while non-cryptographic contract artifacts remain available only behind explicit test flags.
