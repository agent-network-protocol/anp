...
在智能体网络（Agent Network）快速发展的今天，Agent 之间的互联互通变得越来越重要。ANP（Agent Network Protocol）作为一种标准化的智能体通信协议，为 Agent 之间的发现、认证和交互提供了坚实的基础。然而，直接基于 ANP 协议进行开发，开发者需要处理大量的底层细节：DID 认证、JSON-RPC 通信、OpenRPC 文档生成、会话管理等等。
