...
Registry 状态核对时间：**2026-07-13**。Python 和 Rust 的源码 manifest 已准备为 `0.8.9`，但公共 registry 在执行待定发布前仍为 `0.8.8`。Go 暂时保持上一条协调发布线 `v0.8.8`。Dart 单独发布。TypeScript 和 Java 可以从源码或本地构建使用，但本 README **不声称** 它们已经公开发布到 npm 或 Maven Central。

| 语言 | 包 / 模块 | 获取位置 | 已核对版本 | 安装 / 使用 | 示例 | 状态 |
|---|---|---|---|---|---|---|
| Python | `anp` | [PyPI](https://pypi.org/project/anp/) | `0.8.8`（源码已准备 `0.8.9`） | `pip install anp`；OpenANP/FastAPI extras 用 `pip install "anp[api]"` | [examples/python/](examples/python/) | `0.8.9` registry 发布待执行 |
...
| Rust | `anp` | [crates.io](https://crates.io/crates/anp) / [docs.rs](https://docs.rs/anp) | `0.8.8`（源码已准备 `0.8.9`） | `cargo add anp` | [rust/examples/](rust/examples/) | `0.8.9` registry 发布待执行 |
