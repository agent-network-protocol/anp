...
| Go | 已实现 | 仅有 contract/provider surface；没有进程内 OpenMLS status API | 共享 conformance vectors 已通过 |
| Dart | 已实现 | 不适用；Dart 当前没有 Group E2EE/OpenMLS surface | 共享 conformance vectors 已通过 |
| Java | 不适用；Java 当前没有 WNS surface | 不适用；Java 当前没有 Group E2EE/OpenMLS surface | 没有可同步的对应 API |
| TypeScript | 已实现 | 不适用；TypeScript 当前没有 Group E2EE/OpenMLS surface | 共享 conformance vectors 已通过 |

当前 release helper 会统一处理 Python、Rust 和 Go。发版声明仍需区分 WNS generation conformance 与 Rust 进程内 OpenMLS rebind/status 能力。
